...
company_data = pd.read_excel('./phase12/data/startup_data_new.xlsx')
company_data['document'] = company_data.document.apply(lambda x:text_cleaner(x))
big_corpus=noun_corpus(company_data['document'])
...
freq_df.to_csv('./data/zifs_compnay.csv', index=False, encoding='cp949')
...
company_data2 = company_data[['company', 'token','origianl_label', 'predicted_label1', 'predicted_label2', 'predicted_small_label1', 'predicted_small_label2', 'predicted_small_label3']]
company_data2.to_excel('./phase12/data/company_data_new16.xlsx', index=False)

# y_pred = LR_clf.predict(company_doc)
# y_pred_prob = LR_clf.predict_proba(company_doc)
# big_pred_prob = np.argsort(-y_pred_prob, axis=1)
# pred = pd.DataFrame(big_pred_prob[:,0], columns=['predicted_label1'], index=company_data.index)
# two_pred_prob = pd.DataFrame([y_pred_prob[i,two_pred.iloc[i, 0]].tolist() for i in range(two_pred.shape[0])],\
#                              columns = ['predicted_prob1'], index=company_data.index)
#
# company_data = pd.concat([company_data, two_pred, two_pred_prob], axis=1)
# company_data['predicted_label1'] = company_data.predicted_label1.apply(str).map(big_class)
#
# filename_small = './model/ml_model/LR_clf_small_last6.sav'
# LR_clf_small = pickle.load(open(filename_small, 'rb'))
...
# company_data['predicted_small_label3'] = None
# i=0
# sc_candidate = class_dict[company_data.iloc[i]['predicted_label1']]
# sc_candidate = [x for x in sc_candidate if x in key_list]
# sc_candidate_idx = list({key: small_class_dic[key] for key in sc_candidate}.values())
# sc_candidate_idx
# sc_candidate2 = [x for x in small_predict_prob[i] if x in sc_candidate_idx]
# sc_candidate2[:3]
# y_pred_prob_small[0, sc_candidate2[:3]]
#
# for i in range(len(company_data)):
#     sc_candidate = class_dict[company_data.iloc[i]['predicted_label1']]
#     sc_candidate = [x for x in sc_candidate if x in key_list]
...
# company_data['predicted_small_label3'] = company_data.predicted_small_label3.apply(str).map(small_class)
#
# company_data2 = company_data[['company', 'token','origianl_label', 'predicted_label1', 'predicted_small_label1', 'predicted_small_label2', 'predicted_small_label3']]
# company_data2.to_excel('./phase12/data/company_data_new14.xlsx', index=False)
