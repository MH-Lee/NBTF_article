...
glove_model = './model/word_embedding/glove.model'
glove  = Glove.load(glove_model)

def make_glove_data(X, word_vectorizer=glove):
    i, j = 0, 0
    data = []
    for doc in tqdm(X):
        glove_doc_mean = []
        for word in doc:
            try:
                i += 1
                glove_doc_mean.append(word_vectorizer.word_vectors[word_vectorizer.dictionary[word]])
            except KeyError:
                j += 1
                glove_doc_mean.append(np.random.normal(size=300))
        glove_doc_mean = np.array(glove_doc_mean).sum(axis=0)
        data.append(glove_doc_mean)
    print(i, j)
...
X_train = make_glove_data(train_X, glove)
X_test = make_glove_data(test_X, glove)
...
# for i, label in enumerate([big_class_dict, small_class_dict, class_dict]):
#     idx = idx_dict[i]
#     with open('./data/label_dict/{}.json'.format(idx), 'w') as f:
#         json.dump(label, f)
...
filename = './model/ml_model/LR_clf_glove.sav'
...
filename = './model/ml_model/LR_clf_small_glove.sav'
pickle.dump(clf, open(filename, 'wb'))
